...
def _verify_venv(path, target_os='windows'):
    p = _norm(path)
    if not p:
    ...
    if target_os == 'windows':
        ...
        pyexe    = os.path.join(p, 'bin', 'python')
        if os.path.isfile(activate) and os.path.isfile(pyexe):
            return {'ok': True, 'path': p, 'activate': activate, 'python': pyexe}
        return {'ok': False, 'reason': 'No bin/activate or python inside — not a venv.'}
...
def _do_install(cfg):
    ...
    target_os    = cfg.get('os', 'windows')  # 'windows' or 'linux'
    ...
    if python_mode == 'venv':
        vv = _verify_venv(venv_dir, target_os)
        if not vv['ok']:
    ...
    copy_list = list(REQUIRED_SOMNI_FILES) + ['icon.ico', 'icon.png', 'version.txt', 'README.md', 'LICENSE']
    for fname in copy_list:
        ...
        if os.path.isfile(src) and os.path.normcase(src) != os.path.normcase(os.path.join(install_dir, fname)):
            ...
            if target_os == 'linux' and fname.endswith('.sh'):
                os.chmod(os.path.join(install_dir, fname), 0o755)

    if target_os == 'windows':
        _write_windows_scripts(comfy_dir, venv_dir, python_mode, install_dir, boot_delay, open_browser)
        files_written = ['launch_somni.bat', 'launch_comfyui_and_somni.bat', 'somni_config.json']
    else:
        _write_linux_scripts(comfy_dir, venv_dir, python_mode, install_dir, boot_delay, open_browser)
        files_written = ['launch_somni.sh', 'launch_comfyui_and_somni.sh', 'somni_config.json']
...
def _write_windows_scripts(comfy_dir, venv_dir, python_mode, install_dir, boot_delay, open_browser):
...
def _write_linux_scripts(comfy_dir, venv_dir, python_mode, install_dir, boot_delay, open_browser):
    somni_py = sys.executable

    if python_mode == 'portable':
        comfy_start = (
            f'cd "{comfy_dir}" && '
            f'"{os.path.join(comfy_dir, "python_embeded", "python.exe")}" -s main.py'
        )
    elif python_mode == 'venv':
        ...
        comfy_start = f'cd "{comfy_dir}" && python main.py'

    somni_cmd = f'"{somni_py}" server.py'
    if open_browser:
        somni_cmd += ' --open'
    ...
    somni_only_path = os.path.join(install_dir, 'launch_somni.sh')
    with open(somni_only_path, 'w', encoding='utf-8') as f:
        f.write(somni_only)
    os.chmod(somni_only_path, 0o755)

    # launch_comfyui_and_somni.sh
    combined = (
        "#!/bin/bash\n"
        f'cd "{comfy_dir}"\n'
        f'{comfy_start} &\n'
    ...
    combined_path = os.path.join(install_dir, 'launch_comfyui_and_somni.sh')
    with open(combined_path, 'w', encoding='utf-8') as f:
        f.write(combined)
    os.chmod(combined_path, 0o755)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/api/verify-venv':
            target_os = body.get('os', 'windows')
            _send_json(self, _verify_venv(body.get('path', ''), target_os))
            return
