#!/usr/bin/env python3
"""somni installer: serves installer.html on port 8081 and writes launch scripts."""
...
def _verify_install_dir(path):
    ...
    if os.path.isdir(p):
        ...
        # Folder exists but missing files — installer will copy them in
...
def _bat_quote(path):
    """Wrap a path in double quotes for safe use in a .bat command line."""
    return '"' + path.replace('"', '') + '"'


def _do_install(cfg):
    comfy_dir   = _norm(cfg.get('comfyDir'))
    python_mode = cfg.get('pythonMode', 'system')   # 'portable' | 'venv' | 'system'
    ...
    # Copy somni runtime files to the install dir. Installer files (.bat/.py/.html)
    # stay in the setup folder so the install dir is clean.
    ...
    # Use the EXACT Python interpreter that's running this installer for
    # the generated bats. This avoids PATH/MS-Store-stub issues entirely.
    somni_py_quoted = _bat_quote(sys.executable)

    # Build the ComfyUI launch command depending on the env type --------
    if python_mode == 'portable':
        comfy_start = (
    ...
    # Build the somni invocation (optionally with --open flag)
    somni_cmd = f'{somni_py_quoted} server.py'
    if open_browser:
        somni_cmd += ' --open'

    # Write launch_somni.bat -------------------------------------------
    somni_only = (
        "@echo off\r\n"
        "title somni\r\n"
    ...
    # Write launch_comfyui_and_somni.bat -------------------------------
    parts = ["@echo off", "title ComfyUI + somni", ""]
    parts.append(f'start "ComfyUI" cmd /k "{comfy_start}"')
    parts.append("")
    parts.append("REM Wait for ComfyUI to finish booting")
...
def _fetch_latest_release():
    """Returns the parsed JSON of GitHub's 'latest release' for GITHUB_REPO, or None."""
...
def _pick_release_zip_url(info):
    """Prefer an attached .zip asset; fall back to GitHub's auto zipball."""
...
def _do_update_install():
    """Download the latest release zip and apply it over SOMNI_DIR."""
    info = _fetch_latest_release()
    if not info:
        raise RuntimeError('Could not reach GitHub. Check your internet or the GITHUB_REPO constant.')
    url = _pick_release_zip_url(info)
    if not url:
        raise RuntimeError('No release zip found on GitHub for the latest tag.')
    ...
    try:
        ...
        # zipballs from GitHub wrap content in a single OWNER-REPO-SHA/ folder
        entries = os.listdir(tmp_dir)
        root = tmp_dir
        if len(entries) == 1 and os.path.isdir(os.path.join(tmp_dir, entries[0])):
            root = os.path.join(tmp_dir, entries[0])

        # Copy everything over SOMNI_DIR (the setup folder), preserving per-user files
